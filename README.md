...
That flow pre-populates each selected repo with GitHub metadata plus inferred `classifications`. Archa keeps GitHub topics first, supplements them with a locally inferred topic set from repo descriptions, and derives separate high-signal `classifications` from metadata and repo-content inspection.
