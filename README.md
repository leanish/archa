...
That flow pre-populates each selected repo with GitHub metadata plus inferred `classifications`. When GitHub leaves descriptions or topics blank, Archa can fill them from README/source inspection. It keeps GitHub topics first, supplements them with a locally inferred topic set from repo descriptions or inspected repo content, and derives separate high-signal `classifications` from metadata and repo-content inspection.
