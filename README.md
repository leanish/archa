...
`POST /ask` also accepts an optional `attachments` array of `{ "name", "mediaType", "contentBase64" }` objects. Attachments are included in the Codex prompt as untrusted data; text-like files are decoded as UTF-8 and binary files are passed as base64 text. The built-in limits are 8 files, 1 MiB decoded per file, and 3 MiB decoded total.
