...
- config can be bootstrapped from a local catalog file or discovered from a GitHub owner before being selectively added to or overridden in local config, including private repos visible to authenticated GitHub credentials from `GH_TOKEN` / `GITHUB_TOKEN` or the current `gh` login when available, with the same repo-content inspection, size-aware topic enrichment, separate repo classifications, Codex cleanup pass, and progress updates used for both preview and apply
