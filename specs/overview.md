...
- config can be bootstrapped from a local catalog file or discovered from a GitHub owner before being selectively added to or overridden in local config, using authenticated GitHub access from `GH_TOKEN` / `GITHUB_TOKEN` or, if those env vars are unset, the current `gh` login, including private repos visible to that credential; preview hydrates richer metadata for all candidates, while `--apply` starts from repo-name selection and then refines only the selected subset with repo-content inspection, size-aware topic enrichment, separate repo classifications, a Codex cleanup pass, and visible progress updates
